...
// GetMarketPrice gets market price using v2 API
// Updated from /fapi/v1/ticker/price to /fapi/v2/ticker/price
func (t *FuturesTrader) GetMarketPrice(symbol string) (float64, error) {
	// Use v2 API directly via HTTP request
	// v2 API endpoint: /fapi/v2/ticker/price
	...
	// Use SDK's HTTP client if available, otherwise create a new one
...
func (t *FuturesTrader) SetStopLoss(symbol string, positionSide string, quantity, stopPrice float64) error {
	...
	// Format price to correct precision to avoid precision error
...
// GetPricePrecision gets the price precision for a trading pair from PRICE_FILTER
func (t *FuturesTrader) GetPricePrecision(symbol string) (int, error) {
	...
	return 8, nil // Default price precision is 8
}
...
// FormatPrice formats price to correct precision based on PRICE_FILTER
func (t *FuturesTrader) FormatPrice(symbol string, price float64) (string, error) {
	precision, err := t.GetPricePrecision(symbol)
	if err != nil {
		// If retrieval fails, use default format (8 decimal places)
