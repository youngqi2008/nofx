...
func (at *AutoTrader) runCycle() error {
	...
	// 5. Get previous decision record (as context) - only load if previous cycle was successful
	var previousDecisionRecord interface{} = nil
	if at.callCount > 1 && at.store != nil {
		// Get latest 1 record (previous cycle)
		previousRecords, err := at.store.Decision().GetLatestRecords(at.id, 1)
		if err == nil && len(previousRecords) > 0 {
			previousRecord := previousRecords[0]
			// Check if previous cycle was successful
			if previousRecord.Success {
				previousDecisionRecord = previousRecord
				logger.Infof("📋 Loaded previous decision context (Cycle #%d, execution successful)", previousRecord.CycleNumber)
			} else {
				logger.Infof("⏭️ Skipping previous decision context (Cycle #%d, execution failed)", previousRecord.CycleNumber)
			}
		} else if err != nil {
			logger.Infof("⚠️ Failed to get previous decision record: %v", err)
		}
	}

	// 6. Use strategy engine to call AI for decision
	logger.Infof("🤖 Requesting AI analysis and decision... [Strategy Engine]")
	aiDecision, err := kernel.GetFullDecisionWithStrategy(ctx, at.mcpClient, at.strategyEngine, "balanced", previousDecisionRecord)
...
func (at *AutoTrader) buildTradingContext() (*kernel.Context, error) {
	...
	for _, pos := range positions {
		...
		if quantity == 0 {
			// Clean up orphaned orders (stop-loss/take-profit/limit orders) for closed positions
			// This handles cases where TP/SL orders were auto-triggered by exchange,
			// leaving orphaned orders that should be canceled
			openOrders, err := at.trader.GetOpenOrders(symbol)
			if err == nil && len(openOrders) > 0 {
				logger.Infof("🧹 Cleaning up orphaned orders for closed position: %s (found %d open orders)", symbol, len(openOrders))
				if err := at.trader.CancelAllOrders(symbol); err != nil {
					logger.Infof("  ⚠️ Failed to cancel orphaned orders for %s: %v", symbol, err)
				} else {
					logger.Infof("  ✓ Successfully canceled %d orphaned orders for %s", len(openOrders), symbol)
				}
			}

			// Additionally check and cancel Algo Orders (conditional orders) for Binance
			// Algo Orders use a separate API and may not be fully covered by GetOpenOrders
			if at.exchange == "binance" {
				if binanceTrader, ok := at.trader.(*FuturesTrader); ok {
					// Check for Algo Orders separately
					algoOrders, err := binanceTrader.client.NewListOpenAlgoOrdersService().
						Symbol(symbol).
						Do(context.Background())
					if err == nil && len(algoOrders) > 0 {
						logger.Infof("🧹 Cleaning up orphaned Algo Orders (conditional orders) for closed position: %s (found %d Algo orders)", symbol, len(algoOrders))
						if err := binanceTrader.client.NewCancelAllAlgoOpenOrdersService().
							Symbol(symbol).
							Do(context.Background()); err != nil {
							// Ignore "no algo orders" error
							if !strings.Contains(err.Error(), "no algo") && !strings.Contains(err.Error(), "No algo") {
								logger.Infof("  ⚠️ Failed to cancel orphaned Algo Orders for %s: %v", symbol, err)
							}
						} else {
							logger.Infof("  ✓ Successfully canceled %d orphaned Algo Orders for %s", len(algoOrders), symbol)
						}
					}
				}
			}
	...
	if at.store != nil {
		...
		tradesLast24H, err := at.store.Position().GetTradesByTimeRange(at.id, 
			last24HStart.UnixMilli(), now.UnixMilli())
		if err != nil {
			logger.Infof("⚠️ [%s] Failed to get trades in last 24 hours: %v", at.name, err)
		} else {
			logger.Infof("📊 [%s] Found %d trades in last 24 hours", at.name, len(tradesLast24H))
			for _, trade := range tradesLast24H {
				entryTimeStr := ""
				if trade.EntryTime > 0 {
					entryTimeStr = time.Unix(trade.EntryTime, 0).UTC().Format("01-02 15:04 UTC")
				}
				...
				ctx.TradesLast24H = append(ctx.TradesLast24H, kernel.RecentOrder{
					Symbol:       trade.Symbol,
					Side:         trade.Side,
					EntryPrice:   trade.EntryPrice,
					ExitPrice:    trade.ExitPrice,
		...
		// Get trades from today 00:00 to now (natural day)
		todayStart := time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, time.UTC)
		tradesToday, err := at.store.Position().GetTradesByTimeRange(at.id, 
			todayStart.UnixMilli(), now.UnixMilli())
		if err != nil {
			logger.Infof("⚠️ [%s] Failed to get trades today: %v", at.name, err)
		} else {
			logger.Infof("📊 [%s] Found %d trades today (from 00:00 to now)", at.name, len(tradesToday))
			for _, trade := range tradesToday {
				entryTimeStr := ""
				if trade.EntryTime > 0 {
					entryTimeStr = time.Unix(trade.EntryTime, 0).UTC().Format("01-02 15:04 UTC")
				}
				...
				ctx.TradesToday = append(ctx.TradesToday, kernel.RecentOrder{
					Symbol:       trade.Symbol,
					Side:         trade.Side,
					EntryPrice:   trade.EntryPrice,
					ExitPrice:    trade.ExitPrice,
		...
		if len(ctx.TradesLast24H) > 0 {
			ctx.StatsLast24H = at.calculatePeriodStats(ctx.TradesLast24H)
			logger.Infof("📊 [%s] Last 24H stats: %d trades, Net P&L: %.2f USDT, Win Rate: %.1f%%", 
				at.name, ctx.StatsLast24H.TotalTrades, ctx.StatsLast24H.NetPnL, ctx.StatsLast24H.WinRate)
		}

		// Calculate statistics for today
		if len(ctx.TradesToday) > 0 {
			ctx.StatsToday = at.calculatePeriodStats(ctx.TradesToday)
			logger.Infof("📊 [%s] Today stats: %d trades, Net P&L: %.2f USDT, Win Rate: %.1f%%", 
				at.name, ctx.StatsToday.TotalTrades, ctx.StatsToday.NetPnL, ctx.StatsToday.WinRate)
		}
...
// calculatePeriodStats calculates trading statistics for a period from a list of trades
func (at *AutoTrader) calculatePeriodStats(trades []kernel.RecentOrder) *kernel.PeriodTradingStats {
	stats := &kernel.PeriodTradingStats{
		TotalTrades: len(trades),
		MaxWin:      -1e9, // Initialize with very negative value
		MaxLoss:     1e9,  // Initialize with very positive value
	}
	...
	if stats.TotalTrades > 0 {
		stats.WinRate = float64(stats.WinCount) / float64(stats.TotalTrades) * 100
		stats.AvgPnL = stats.NetPnL / float64(stats.TotalTrades)
	}

	// If no wins, set MaxWin to 0
	if stats.WinCount == 0 {
		stats.MaxWin = 0
	}
	// If no losses, set MaxLoss to 0
