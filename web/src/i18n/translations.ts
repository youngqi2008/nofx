...
export const translations = {
  en: {
    ...
    // Footer
    footerTitle: 'Ares - AI Trading System',
    ...
    heroDescription:
      'Ares is the future standard for AI trading — an open, community-driven agentic trading OS. Supporting Binance, Aster DEX and other exchanges, self-hosted, multi-agent competition, let AI automatically make decisions, execute and optimize trades for you.',
    poweredBy: 'Powered by Aster DEX and Binance.',

    // Landing Page CTA
    readyToDefine: 'Ready to define the future of AI trading?',
    startWithCrypto:
      'Starting with crypto markets, expanding to TradFi. Ares is the infrastructure of AgentFi.',
    getStartedNow: 'Get Started Now',
    viewSourceCode: 'View Source Code',

    // Features Section
    coreFeatures: 'Core Features',
    whyChooseNofx: 'Why Choose Ares?',
    openCommunityDriven:
    ...
    openSourceFeatures1: '企业级安全保障',
    openSourceFeatures2: '数据加密存储',
    ...
    // About Section
    aboutNofx: 'About Ares',
    whatIsNofx: 'What is Ares?',
    aresNotAnotherBot:
      "Ares 是新一代 AI 交易平台 —",
    aresDescription1:
      '专业的智能交易系统，提供统一的',
    aresDescription2:
      "'决策-风险-执行' 层，支持所有资产类别。",
    aresDescription3:
      '从加密货币市场开始（7x24小时，高波动性），未来扩展到股票、期货、外汇。核心：开放架构，AI',
    aresDescription4:
      '达尔文主义（多代理自竞争，策略进化），智能',
    aresDescription5:
      '飞轮（持续优化与进化）。',
    youFullControl: '您完全掌控',
    fullControlDesc: '完全控制 AI 提示词和资金',

    // How It Works Section
    howToStart: 'How to Get Started with Ares',
    fourSimpleSteps:
      'Three simple steps to start your AI automated trading journey',
    step1Title: 'Create Account',
    step1Desc:
      'Quick registration to start your AI trading journey immediately.',
    step2Title: 'Configure Trading',
    step2Desc:
      'Connect exchange account, select AI model, set trading strategy parameters.',
    step3Title: 'Start AI Trading',
    step3Desc:
      'Create trader, AI will automatically analyze markets and execute trades. Monitor returns in real-time.',
    importantRiskWarning: 'Important Risk Warning',
    riskWarningText:
      'Dev branch is unstable, do not use funds you cannot afford to lose. Ares is non-custodial, no official strategies. Trading involves risks, invest carefully.',
    ...
    // Login Modal
    accessNofxPlatform: 'Access Ares Platform',
    loginRegisterPrompt:
    ...
    faqSubtitle: 'Find answers to common questions about Ares',
    ...
    // ===== GETTING STARTED =====
    faqWhatIsAres: 'What is Ares?',
    faqWhatIsAresAnswer:
      'Ares is an AI-powered trading platform for cryptocurrency and US stock markets. It uses large language models (LLMs) like DeepSeek, GPT, Claude, Gemini to analyze market data and make autonomous trading decisions. Key features include: multi-AI model support, multi-exchange trading, visual strategy builder, backtesting, and AI debate arena for consensus decisions.',

    faqHowDoesItWork: 'How does Ares work?',
    faqHowDoesItWorkAnswer:
      'Ares works in 5 steps: 1) Configure AI models and exchange API credentials; 2) Create a trading strategy (coin selection, indicators, risk controls); 3) Create a "Trader" combining AI model + Exchange + Strategy; 4) Start the trader - it will analyze market data at regular intervals and make buy/sell/hold decisions; 5) Monitor performance on the dashboard. The AI uses Chain of Thought reasoning to explain each decision.',

    faqIsProfitable: 'Is Ares profitable?',
    faqIsProfitableAnswer:
      'AI trading is experimental and NOT guaranteed to be profitable. Cryptocurrency futures are highly volatile and risky. Ares is designed for educational and research purposes. We strongly recommend: starting with small amounts (10-50 USDT), never investing more than you can afford to lose, thoroughly testing with backtests before live trading, and understanding that past performance does not guarantee future results.',
    ...
    faqSystemRequirementsAnswer:
      'Ares is a cloud-based platform accessible via web browser. No local installation required. All you need is a modern browser (Chrome, Firefox, Safari, Edge) and a stable internet connection. The platform runs on our secure servers, ensuring 24/7 availability and automatic updates.',
    ...
    faqStopLossTakeProfit: 'Does Ares support stop-loss and take-profit?',
    ...
    faqMultipleTradersAnswer:
      'Yes! Ares supports running 20+ concurrent traders. Each trader can have different: AI model, exchange account, strategy, decision interval. Use this to A/B test strategies, compare AI models, or diversify across exchanges. Monitor all traders on the Competition page.',
    ...
    faqPortInUseAnswer:
      'Check what\'s using the port: "lsof -i :8080" (macOS/Linux) or "netstat -ano | findstr 8080" (Windows). Kill the process or change ports in .env: Ares_BACKEND_PORT=8081, Ares_FRONTEND_PORT=3001. Restart with "docker compose down && docker compose up -d".',

    faqFrontendNotLoading: 'Frontend shows "Loading..." forever',
    faqFrontendNotLoadingAnswer:
      'Backend may not be running or reachable. Check: 1) "curl http://127.0.0.1:8080/api/health" should return {"status":"ok"}; 2) "docker compose ps" to verify containers are running; 3) Check backend logs: "docker compose logs ares-backend"; 4) Ensure firewall allows port 8080.',

    faqDatabaseLocked: 'Database locked error',
    faqDatabaseLockedAnswer:
      'Multiple processes accessing SQLite simultaneously. Solution: 1) Stop all processes: "docker compose down" or "pkill ares"; 2) Remove lock files if present: "rm -f data/data.db-wal data/data.db-shm"; 3) Restart: "docker compose up -d". Only one backend instance should access the database.',

    faqTALibNotFound: 'TA-Lib not found during build',
    faqTALibNotFoundAnswer:
      'TA-Lib is required for technical indicators. Install: macOS: "brew install ta-lib"; Ubuntu/Debian: "sudo apt-get install libta-lib0-dev"; CentOS: "yum install ta-lib-devel". After installing, rebuild: "go build -o ares". Docker images include TA-Lib pre-installed.',
    ...
    faqBinancePositionModeAnswer:
      'Error: "Order\'s position side does not match user\'s setting". You\'re in One-way Mode but Ares requires Hedge Mode. Fix: 1) Close ALL positions first; 2) Binance Futures → Settings (gear icon) → Preferences → Position Mode → Switch to "Hedge Mode" (双向持仓); 3) Restart your trader.',

    faqBalanceShowsZero: 'Account balance shows 0',
    faqBalanceShowsZeroAnswer:
      'Funds are likely in Spot wallet, not Futures wallet. Solution: 1) In Binance, go to Wallet → Futures → Transfer; 2) Transfer USDT from Spot to Futures; 3) Refresh Ares dashboard. Also check: funds not locked in savings/staking products.',
    ...
    faqEncryptionDetails: 'What encryption does Ares use?',
    faqEncryptionDetailsAnswer:
      'Ares uses multiple encryption layers: 1) AES-256-GCM for database storage (API keys, secrets); 2) RSA-2048 for optional transport encryption (browser to server); 3) JWT for authentication tokens. Keys are generated during installation. Enable TRANSPORT_ENCRYPTION=true for HTTPS environments.',

    faqSecurityBestPractices: 'What are security best practices?',
    faqSecurityBestPracticesAnswer:
      'Recommended: 1) Use exchange API keys with IP whitelist and minimal permissions (Futures Trading only); 2) Use dedicated subaccount for Ares; 3) Enable TRANSPORT_ENCRYPTION for remote deployments; 4) Never share .env or data.db files; 5) Use HTTPS with valid certificates; 6) Regularly rotate API keys; 7) Monitor account activity.',

    faqCanAresStealFunds: 'Can Ares steal my funds?',
    faqCanAresStealFundsAnswer:
      'API 密钥存储在您的机器本地，从不发送到外部服务器。Ares 只有您通过 API 密钥授予的权限。为最大安全：使用仅交易权限（无提现）的 API 密钥，启用 IP 白名单，使用专用子账户。',
    ...
    faqCustomAIAPIAnswer:
      'Yes! Ares supports any OpenAI-compatible API. In Config → AI Models → Custom API: 1) Enter your API endpoint URL (e.g., https://your-api.com/v1); 2) Enter API key; 3) Specify model name. This works with self-hosted models, alternative providers, or Claude via third-party proxies.',

    faqAIHallucinations: 'What about AI hallucinations?',
    faqAIHallucinationsAnswer:
      'AI models can sometimes produce incorrect or fabricated information ("hallucinations"). Ares mitigates this by: 1) Providing structured prompts with real market data; 2) Enforcing JSON output format for decisions; 3) Validating orders before execution. However, AI trading is experimental - always monitor decisions and don\'t rely solely on AI judgment.',
    ...
    // ===== CONTRIBUTING =====
    faqHowToContribute: 'How can I get support?',
    faqHowToContributeAnswer:
      'Ares 持续优化与进化。如有问题或建议，请联系我们的支持团队。',

    faqPRGuidelines: 'How do I report issues?',
    faqPRGuidelinesAnswer:
      '如有问题，请联系我们的支持团队，提供：1）问题清晰描述；2）复现步骤；3）预期 vs 实际行为；4）系统信息（浏览器版本等）；5）相关日志。',

    faqBountyProgram: 'Is there customer support?',
    faqBountyProgramAnswer:
      '是的！我们提供专业的客户支持服务。如有任何问题或需要帮助，请通过平台内的支持渠道联系我们。',
    ...
    environmentCheck: {
      ...
      unsupportedDesc:
        'Open Ares over HTTPS (or http://localhost during development) and avoid insecure iframes/reverse proxies so the browser can enable Web Crypto.',
  ...
  zh: {
    ...
    heroTitle2: 'Write the Trade.',
    heroDescription:
      'Ares 是 AI 交易的未来标准——一个开放、社区驱动的代理式交易操作系统。支持 Binance、Aster DEX 等交易所，自托管、多代理竞争，让 AI 为你自动决策、执行和优化交易。',
    poweredBy: '由 Aster DEX 和 Binance 提供支持。',

    // Landing Page CTA
    readyToDefine: '准备好定义 AI 交易的未来吗？',
    startWithCrypto:
      '从加密市场起步，扩展到 TradFi。Ares 是 AgentFi 的基础架构。',
    getStartedNow: '立即开始',

    // Features Section
    coreFeatures: '核心功能',
    whyChooseNofx: '为什么选择 Ares？',
    ...
    aboutNofx: '关于 Ares',
    whatIsNofx: '什么是 Ares？',
    aresNotAnotherBot: "Ares 是新一代 AI 交易平台 —",
    aresDescription1: "专业的智能交易系统，提供统一的 '决策-风险-执行'",
    aresDescription2: '层，支持所有资产类别。',
    aresDescription3:
      '从加密货币市场开始（7x24小时，高波动性），未来扩展到股票、期货、外汇。核心：开放架构，AI',
    aresDescription4:
      '达尔文主义（多代理自竞争，策略进化），智能',
    aresDescription5: '飞轮（持续优化与进化）。',
    youFullControl: '您完全掌控',
    fullControlDesc: '完全控制 AI 提示词和资金',

    // How It Works Section
    howToStart: '如何开始使用 Ares',
    fourSimpleSteps: '三个简单步骤，开启 AI 自动交易之旅',
    step1Title: '注册账户',
    step1Desc:
      '快速注册，立即开始您的 AI 交易之旅。',
    step2Title: '配置交易环境',
    step2Desc:
      '连接交易所账户，选择 AI 模型，设置交易策略参数。',
    step3Title: '启动 AI 交易',
    step3Desc:
      '创建交易员，AI 将自动分析市场并执行交易。实时监控收益。',
    importantRiskWarning: '重要风险提示',
    riskWarningText:
      'dev 分支不稳定，勿用无法承受损失的资金。Ares 非托管，无官方策略。交易有风险，投资需谨慎。',
    ...
    // ===== 入门指南 =====
    faqWhatIsAres: 'Ares 是什么？',
    faqWhatIsAresAnswer:
      'Ares 是一个开源的 AI 驱动交易操作系统，支持加密货币和美股市场。它使用大语言模型（LLM）如 DeepSeek、GPT、Claude、Gemini 来分析市场数据，进行自主交易决策。核心功能包括：多 AI 模型支持、多交易所交易、可视化策略构建器、回测系统、以及用于共识决策的 AI 辩论竞技场。',

    faqHowDoesItWork: 'Ares 是如何工作的？',
    faqHowDoesItWorkAnswer:
      'Ares 分 5 步工作：1）配置 AI 模型和交易所 API 凭证；2）创建交易策略（币种选择、指标、风控）；3）创建"交易员"，组合 AI 模型 + 交易所 + 策略；4）启动交易员 - 它会定期分析市场数据并做出买入/卖出/持有决策；5）在仪表板上监控表现。AI 使用思维链（Chain of Thought）推理来解释每个决策。',

    faqIsProfitable: 'Ares 能盈利吗？',
    faqIsProfitableAnswer:
      'AI 交易是实验性的，不保证盈利。加密货币期货波动性大、风险高。Ares 仅用于教育和研究目的。我们强烈建议：从小额开始（10-50 USDT），不要投入超过承受能力的资金，在实盘交易前充分回测，并理解过去的表现不代表未来的结果。',
    ...
    faqSupportedAIModelsAnswer:
      'Ares 支持 7+ 种 AI 模型：DeepSeek（推荐性价比）、阿里云通义千问、OpenAI（GPT-5.2）、Anthropic Claude、Google Gemini、xAI Grok、Kimi（月之暗面）。您也可以使用任何 OpenAI 兼容的 API 端点。每个模型各有优势 - DeepSeek 性价比高，OpenAI 能力强但贵，Claude 擅长推理。',

    faqSystemRequirements: '系统要求是什么？',
    faqSystemRequirementsAnswer:
      'Ares 是基于云端的平台，通过网页浏览器访问。无需本地安装。您只需要现代浏览器（Chrome、Firefox、Safari、Edge）和稳定的网络连接。平台运行在我们的安全服务器上，确保 7x24 小时可用性和自动更新。',
    ...
    faqHyperliquidSetupAnswer:
      'Hyperliquid 是去中心化交易所，需要钱包认证。步骤：1）访问 app.hyperliquid.xyz；2）连接钱包；3）生成 API 钱包（推荐）或使用主钱包；4）复制钱包地址和私钥；5）在 Ares 中添加 Hyperliquid 交易所并填入凭证。无需 KYC，完全链上。',
    ...
    faqStopLossTakeProfit: 'Ares 支持止损止盈吗？',
    faqStopLossTakeProfitAnswer:
    ...
    faqMultipleTradersAnswer:
      '可以！Ares 支持运行 20+ 个并发交易员。每个交易员可以有不同的：AI 模型、交易所账户、策略、决策间隔。用于 A/B 测试策略、比较 AI 模型或跨交易所分散风险。在竞赛页面监控所有交易员。',
    ...
    faqPortInUseAnswer:
      '查看占用端口的进程：macOS/Linux 用 "lsof -i :8080"，Windows 用 "netstat -ano | findstr 8080"。终止进程或在 .env 中修改端口：Ares_BACKEND_PORT=8081、Ares_FRONTEND_PORT=3001。然后 "docker compose down && docker compose up -d" 重启。',

    faqFrontendNotLoading: '前端一直显示"加载中..."',
    faqFrontendNotLoadingAnswer:
      '后端可能未运行或无法访问。检查：1）"curl http://127.0.0.1:8080/api/health" 应返回 {"status":"ok"}；2）"docker compose ps" 验证容器运行中；3）查看后端日志："docker compose logs ares-backend"；4）确保防火墙允许 8080 端口。',

    faqDatabaseLocked: '数据库锁定错误',
    faqDatabaseLockedAnswer:
      '多个进程同时访问 SQLite 导致。解决方案：1）停止所有进程："docker compose down" 或 "pkill ares"；2）如有锁文件删除："rm -f data/data.db-wal data/data.db-shm"；3）重启："docker compose up -d"。只能有一个后端实例访问数据库。',

    faqTALibNotFound: '构建时找不到 TA-Lib',
    faqTALibNotFoundAnswer:
      'TA-Lib 是技术指标所需。安装：macOS："brew install ta-lib"；Ubuntu/Debian："sudo apt-get install libta-lib0-dev"；CentOS："yum install ta-lib-devel"。安装后重新构建："go build -o ares"。Docker 镜像已预装 TA-Lib。',
    ...
    faqBinancePositionModeAnswer:
      '错误："Order\'s position side does not match user\'s setting"。您处于单向持仓模式，但 Ares 需要双向持仓模式。修复：1）先平掉所有持仓；2）币安合约 → 设置（齿轮图标）→ 偏好设置 → 持仓模式 → 切换为"双向持仓"；3）重启交易员。',

    faqBalanceShowsZero: '账户余额显示 0',
    faqBalanceShowsZeroAnswer:
      '资金可能在现货钱包而非合约钱包。解决方案：1）在币安进入钱包 → 合约 → 划转；2）将 USDT 从现货划转到合约；3）刷新 Ares 仪表板。也检查：资金未被理财/质押产品锁定。',
    ...
    faqEncryptionDetails: 'Ares 使用什么加密？',
    faqEncryptionDetailsAnswer:
      'Ares 使用多层加密：1）AES-256-GCM 用于数据库存储（API 密钥、密钥）；2）RSA-2048 用于可选的传输加密（浏览器到服务器）；3）JWT 用于认证令牌。密钥在安装时生成。HTTPS 环境启用 TRANSPORT_ENCRYPTION=true。',

    faqSecurityBestPractices: '安全最佳实践是什么？',
    faqSecurityBestPracticesAnswer:
      '建议：1）使用带 IP 白名单和最小权限（仅合约交易）的交易所 API 密钥；2）为 Ares 使用专用子账户；3）远程部署启用 TRANSPORT_ENCRYPTION；4）切勿分享 .env 或 data.db 文件；5）使用有效证书的 HTTPS；6）定期轮换 API 密钥；7）监控账户活动。',

    faqCanAresStealFunds: 'Ares 会盗取我的资金吗？',
    faqCanAresStealFundsAnswer:
      'Ares 是开源的（AGPL-3.0 许可）- 您可以在 GitHub 审计所有代码。API 密钥存储在您的机器本地，从不发送到外部服务器。Ares 只有您通过 API 密钥授予的权限。为最大安全：使用仅交易权限（无提现）的 API 密钥，启用 IP 白名单，使用专用子账户。',
    ...
    faqCustomAIAPIAnswer:
      '可以！Ares 支持任何 OpenAI 兼容的 API。在配置 → AI 模型 → 自定义 API 中：1）输入 API 端点 URL（如 https://your-api.com/v1）；2）输入 API 密钥；3）指定模型名称。适用于自托管模型、替代提供商或通过第三方代理的 Claude。',

    faqAIHallucinations: 'AI 幻觉问题怎么办？',
    faqAIHallucinationsAnswer:
      'AI 模型有时会产生不正确或虚构的信息（"幻觉"）。Ares 通过以下方式缓解：1）提供带真实市场数据的结构化提示词；2）强制 JSON 输出格式；3）执行前验证订单。但 AI 交易是实验性的 - 始终监控决策，不要完全依赖 AI 判断。',
    ...
    environmentCheck: {
      ...
      unsupportedDesc:
        '请通过 HTTPS 或本机 localhost 访问 Ares，并避免嵌入不安全 iframe/反向代理，以符合浏览器的 Web Crypto 规则。',
