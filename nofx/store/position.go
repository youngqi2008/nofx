...
// GetTradesByTimeRange gets closed trades within a time range
// startTimeMs and endTimeMs are Unix milliseconds UTC
func (s *PositionStore) GetTradesByTimeRange(traderID string, startTimeMs, endTimeMs int64) ([]RecentTrade, error) {
	var positions []TraderPosition
	err := s.db.Where("trader_id = ? AND status = ? AND exit_time >= ? AND exit_time <= ?", 
		traderID, "CLOSED", startTimeMs, endTimeMs).
		Order("exit_time DESC").
		Find(&positions).Error
	if err != nil {
		return nil, fmt.Errorf("failed to query trades by time range: %w", err)
	}
