...
func (d *Database) createTables() error {
	...
	alterQueries := []string{
		...
		`ALTER TABLE traders ADD COLUMN indicator_config TEXT DEFAULT ''`,               // 前端指标/周期配置 JSON
...
type TraderRecord struct {
	...
	IndicatorConfig      string    `json:"indicator_config"`       // 前端选中的时间序列与指标（JSON，如 {"timeframes":["3m","4h"],"indicators":["ema","macd","rsi"]}）
	CreatedAt            time.Time `json:"created_at"`
	UpdatedAt            time.Time `json:"updated_at"`
}
