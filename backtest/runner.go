...
func (r *Runner) buildDecisionContext(ts int64, marketData map[string]*market.Data, multiTF map[string]map[string]*market.Data, priceMap map[string]float64, callCount int) (*kernel.Context, *store.DecisionRecord, error) {
	...
	// Load recent completed trades and convert to RecentOrders (matching live trading behavior)
	r.loadRecentOrders(ctx, ts)

	// Calculate trading statistics (matching live trading behavior)
	r.calculateTradingStats(ctx, ts)
...
func (r *Runner) invokeAIWithRetry(ctx *kernel.Context) (*kernel.FullDecision, error) {
	// Get previous decision record (matching live trading behavior)
	var previousDecisionRecord interface{} = nil
	state := r.snapshotState()
	currentCycle := state.DecisionCycle + 1 // Current cycle being processed
	if currentCycle > 1 {
		// Load the previous cycle's decision record (currentCycle - 1)
		previousCycle := currentCycle - 1
		previousRecord, err := LoadDecisionTrace(r.cfg.RunID, previousCycle)
		if err == nil && previousRecord != nil {
			// Only use if previous cycle was successful (matching live trading behavior)
			if previousRecord.Success {
				previousDecisionRecord = previousRecord
				logger.Infof("📋 Backtest: Loaded previous decision context (Cycle #%d, execution successful)", previousRecord.CycleNumber)
			} else {
				logger.Infof("⏭️ Backtest: Skipping previous decision context (Cycle #%d, execution failed)", previousRecord.CycleNumber)
			}
		} else if err != nil {
			logger.Infof("⚠️ Backtest: Failed to get previous decision record (Cycle #%d): %v", previousCycle, err)
		}
	}
	...
	for attempt := 0; attempt < aiDecisionMaxRetries; attempt++ {
		...
		fd, err := kernel.GetFullDecisionWithStrategy(
			...
			r.strategyEngine,
			r.cfg.PromptVariant,
			previousDecisionRecord, // ✅ Now includes previous decision context (matching live trading)
		)
		if err == nil {
			return fd, nil
		}
...
// loadRecentOrders loads recent completed trades from storage and converts them to RecentOrders
// This matches the behavior of live trading where recent trades are included in the user prompt
func (r *Runner) loadRecentOrders(ctx *kernel.Context, currentTS int64) {
	// Load all trade events up to current timestamp
	events, err := LoadTradeEvents(r.cfg.RunID)
	if err != nil {
		logger.Infof("⚠️ Backtest: Failed to load trade events: %v", err)
		return
	}

	// Filter events up to current timestamp and find completed trades (open + close pairs)
	...
	// Use a queue-like structure to handle FIFO matching for multiple opens/closes
	type positionQueue struct {
		positions []*openPosition
	}
	openQueues := make(map[string]*positionQueue) // key: symbol:side
	completedTrades := make([]kernel.RecentOrder, 0)

	// Process events in chronological order to pair open/close (FIFO matching)
	for _, evt := range events {
		if evt.Timestamp > currentTS {
			continue // Skip future events
		}

		key := fmt.Sprintf("%s:%s", evt.Symbol, evt.Side)
		action := strings.ToLower(evt.Action)

		if action == "open_long" || action == "open_short" {
			// Add to open queue
			if openQueues[key] == nil {
				openQueues[key] = &positionQueue{positions: make([]*openPosition, 0)}
			}
			openQueues[key].positions = append(openQueues[key].positions, &openPosition{
				Symbol:     evt.Symbol,
				Side:       evt.Side,
			...
			// Only process fully closed positions (PositionAfter <= 1e-8) for accuracy
			// Partial closes are more complex and would require tracking average entry prices
			if evt.PositionAfter > 1e-8 {
				continue // Skip partial closes for now
			}

			// Match with oldest open position (FIFO)
			queue, ok := openQueues[key]
			if !ok || len(queue.positions) == 0 {
				continue // No matching open position
			}

			closeQty := evt.Quantity
			remainingCloseQty := closeQty

			// Process closes against open positions (FIFO) until fully matched
			for remainingCloseQty > 1e-8 && len(queue.positions) > 0 {
				openPos := queue.positions[0]
				matchedQty := math.Min(remainingCloseQty, openPos.Quantity)

				// For fully closed positions, use the actual RealizedPnL from the event
				// For partial matches, calculate proportional PnL
				var matchedPnL float64
				if matchedQty >= openPos.Quantity-1e-8 {
					// Fully matched this open position - use proportional RealizedPnL
					// Since we only process fully closed positions (PositionAfter <= 1e-8),
					// the entire RealizedPnL belongs to this close
					matchedPnL = evt.RealizedPnL
				} else {
					// Partial match - calculate proportional PnL
					closePortion := matchedQty / closeQty
					matchedPnL = evt.RealizedPnL * closePortion
				}

				matchedEntryValue := openPos.EntryPrice * matchedQty
				pnlPct := 0.0
				if matchedEntryValue > 0 {
					pnlPct = (matchedPnL / matchedEntryValue) * 100
				}

				// Calculate hold duration
				holdDurationMs := evt.Timestamp - openPos.EntryTime
				holdDuration := formatDuration(holdDurationMs)

				// Format times
				entryTimeStr := time.UnixMilli(openPos.EntryTime).UTC().Format("01-02 15:04 UTC")
				exitTimeStr := time.UnixMilli(evt.Timestamp).UTC().Format("01-02 15:04 UTC")

				completedTrades = append(completedTrades, kernel.RecentOrder{
					Symbol:       evt.Symbol,
					Side:         strings.ToUpper(evt.Side),
				...
				// Update quantities
				openPos.Quantity -= matchedQty
				remainingCloseQty -= matchedQty

				// Remove fully closed positions from queue
				if openPos.Quantity <= 1e-8 {
					queue.positions = queue.positions[1:]
				}
			}
		}
	}

	// Sort by exit time (most recent first) and take last 10
	sort.Slice(completedTrades, func(i, j int) bool {
		return completedTrades[i].ExitTime > completedTrades[j].ExitTime
	})
	if len(completedTrades) > 10 {
		completedTrades = completedTrades[:10]
	}

	ctx.RecentOrders = completedTrades
	if len(completedTrades) > 0 {
		logger.Infof("📊 Backtest: Found %d recent completed trades for AI context", len(completedTrades))
	}
}

// calculateTradingStats calculates trading statistics from trade events
// This matches the behavior of live trading where trading stats are included in the user prompt
func (r *Runner) calculateTradingStats(ctx *kernel.Context, currentTS int64) {
	// Load all trade events up to current timestamp
	events, err := LoadTradeEvents(r.cfg.RunID)
	if err != nil {
		logger.Infof("⚠️ Backtest: Failed to load trade events for stats: %v", err)
		return
	}

	// Filter completed trades (those with realized PnL)
	completedTrades := make([]TradeEvent, 0)
	for _, evt := range events {
		if evt.Timestamp <= currentTS && evt.RealizedPnL != 0 {
			completedTrades = append(completedTrades, evt)
		}
	}
	...
	winCount := 0
	lossCount := 0
	pnls := make([]float64, 0, len(completedTrades))
	...
	stats := &kernel.TradingStats{
		TotalTrades: len(completedTrades),
	}

	if len(completedTrades) > 0 {
		stats.WinRate = float64(winCount) / float64(len(completedTrades)) * 100
	}

	if totalLoss > 0 {
		stats.ProfitFactor = totalWin / totalLoss
	} else if totalWin > 0 {
		stats.ProfitFactor = 100.0 // No losses but have wins
	}

	if winCount > 0 {
		stats.AvgWin = totalWin / float64(winCount)
	}

	if lossCount > 0 {
		stats.AvgLoss = totalLoss / float64(lossCount)
	}

	// Calculate Sharpe ratio from PnL sequence
	if len(pnls) > 1 {
		stats.SharpeRatio = calculateSharpeFromPnls(pnls)
	}

	// Calculate max drawdown from equity points
	equityPoints, err := LoadEquityPoints(r.cfg.RunID)
	if err == nil && len(equityPoints) > 0 {
		// Filter equity points up to current timestamp
		filteredPoints := make([]EquityPoint, 0)
		for _, pt := range equityPoints {
			if pt.Timestamp <= currentTS {
				filteredPoints = append(filteredPoints, pt)
			}
		}
		if len(filteredPoints) > 0 {
			stats.MaxDrawdownPct = calculateMaxDrawdownFromEquity(filteredPoints)
		}
	}

	stats.TotalPnL = totalPnL
	ctx.TradingStats = stats

	logger.Infof("📊 Backtest: Calculated trading stats: %d trades, Win Rate: %.1f%%, Profit Factor: %.2f, Sharpe: %.2f",
		stats.TotalTrades, stats.WinRate, stats.ProfitFactor, stats.SharpeRatio)
}

// formatDuration formats milliseconds to human-readable duration string
...
// calculateSharpeFromPnls calculates Sharpe ratio from a sequence of PnL values
func calculateSharpeFromPnls(pnls []float64) float64 {
	if len(pnls) < 2 {
		return 0
	}
	...
	// Annualize assuming ~252 trading days and daily returns
	// For backtest, we approximate by scaling by sqrt(252)
...
// calculateMaxDrawdownFromEquity calculates max drawdown from equity points
func calculateMaxDrawdownFromEquity(points []EquityPoint) float64 {
	if len(points) == 0 {
		return 0
	}
