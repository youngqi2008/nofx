...
// PeriodTradingStats trading statistics for a specific time period
type PeriodTradingStats struct {
	TotalTrades   int     `json:"total_trades"`   // Total number of trades
	NetPnL        float64 `json:"net_pnl"`        // Net profit/loss (净盈亏)
	TotalProfit   float64 `json:"total_profit"`    // Total profit (only winning trades)
	TotalLoss     float64 `json:"total_loss"`     // Total loss (only losing trades)
	WinCount      int     `json:"win_count"`      // Number of winning trades
	LossCount     int     `json:"loss_count"`     // Number of losing trades
	WinRate       float64 `json:"win_rate"`        // Win rate (%)
	AvgPnL        float64 `json:"avg_pnl"`         // Average P&L per trade
	MaxWin        float64 `json:"max_win"`         // Maximum single win
	MaxLoss       float64 `json:"max_loss"`       // Maximum single loss
}
...
type Context struct {
	...
	TradesLast24H   []RecentOrder                      `json:"trades_last_24h,omitempty"`   // Trades in the last 24 hours
	TradesToday      []RecentOrder                      `json:"trades_today,omitempty"`      // Trades from today 00:00 to now
	StatsLast24H     *PeriodTradingStats                `json:"stats_last_24h,omitempty"`    // Statistics for last 24 hours
	StatsToday       *PeriodTradingStats                 `json:"stats_today,omitempty"`       // Statistics for today (from 00:00 to now)
...
// previousDecisionRecord: optional parameter for previous decision record to include in user prompt as context
func GetFullDecisionWithStrategy(ctx *Context, mcpClient mcp.AIClient, engine *StrategyEngine, variant string, previousDecisionRecord interface{}) (*FullDecision, error) {
	if ctx == nil {
	...
	// 3. Build User Prompt using strategy engine (with previous decision context if provided)
	userPrompt := engine.BuildUserPrompt(ctx, previousDecisionRecord)
...
// previousDecisionRecord: optional parameter for previous decision record to include as context
func (e *StrategyEngine) BuildUserPrompt(ctx *Context, previousDecisionRecord interface{}) string {
	var sb strings.Builder
	...
	// Trades in the last 24 hours (detailed list)
	if len(ctx.TradesLast24H) > 0 {
		...
		for i, order := range ctx.TradesLast24H {
			resultStr := "Profit"
			if order.RealizedPnL < 0 {
				resultStr = "Loss"
			}
	...
	// Trading statistics for last 24 hours
	if ctx.StatsLast24H != nil && ctx.StatsLast24H.TotalTrades > 0 {
		lang := e.GetLanguage()
		if lang == LangChinese {
			sb.WriteString("## 过去24小时交易统计\n")
			sb.WriteString(fmt.Sprintf("总交易数: %d 笔 | 净盈亏: %+.2f USDT | 胜率: %.1f%% | 平均盈亏: %+.2f USDT/笔\n",
				ctx.StatsLast24H.TotalTrades,
				ctx.StatsLast24H.NetPnL,
				ctx.StatsLast24H.WinRate,
				ctx.StatsLast24H.AvgPnL))
		} else {
			sb.WriteString("## Trading Statistics (Last 24 Hours)\n")
			sb.WriteString(fmt.Sprintf("Total Trades: %d | Net P&L: %+.2f USDT | Win Rate: %.1f%% | Avg P&L: %+.2f USDT/trade\n",
				ctx.StatsLast24H.TotalTrades,
				ctx.StatsLast24H.NetPnL,
				ctx.StatsLast24H.WinRate,
				ctx.StatsLast24H.AvgPnL))
	...
	if len(ctx.TradesToday) > 0 {
		...
		for i, order := range ctx.TradesToday {
			resultStr := "Profit"
			if order.RealizedPnL < 0 {
				resultStr = "Loss"
			}
	...
	if ctx.StatsToday != nil && ctx.StatsToday.TotalTrades > 0 {
		...
		if lang == LangChinese {
			...
			sb.WriteString(fmt.Sprintf("总交易数: %d 笔 | 净盈亏: %+.2f USDT | 胜率: %.1f%% | 平均盈亏: %+.2f USDT/笔\n",
				ctx.StatsToday.TotalTrades,
				ctx.StatsToday.NetPnL,
				ctx.StatsToday.WinRate,
				ctx.StatsToday.AvgPnL))
		} else {
	...
	if previousDecisionRecord != nil {
		...
		if lang == LangChinese {
			...
			sb.WriteString("## 🔄 Previous Decision Context\n\n")
		}

		// Try to extract information from decision record using JSON serialization/deserialization
			...
			if err := json.Unmarshal(jsonData, &recordMap); err == nil {
				...
				if cycleNum, ok := recordMap["cycle_number"].(float64); ok {
					if lang == LangChinese {
						...
						sb.WriteString(fmt.Sprintf("**Cycle #%.0f**", int(cycleNum)))
					}
						...
						if timestampStr != "" {
							...
							if t, err := time.Parse(time.RFC3339, timestampStr); err == nil {
								if lang == LangChinese {
									...
									sb.WriteString(fmt.Sprintf(" (Time: %s)", t.Format("2006-01-02 15:04:05")))
								}
							} else {
								...
								if !parsed {
									// If cannot parse, display original string (truncated)
									...
									if lang == LangChinese {
										...
										sb.WriteString(fmt.Sprintf(" (Time: %s)", timestampStr))
									}
								}
							}
						}
				...
				// Display chain of thought (if exists)
				if cotTrace, ok := recordMap["cot_trace"].(string); ok && cotTrace != "" {
					// Limit CoT length to avoid prompt being too long
					maxCoTLength := 2000
					cotDisplay := cotTrace
					if len(cotDisplay) > maxCoTLength {
						if lang == LangChinese {
							...
							cotDisplay = cotDisplay[:maxCoTLength] + "\n\n... (Chain of thought truncated, showing first 2000 characters)"
						}
					}
					if lang == LangChinese {
						sb.WriteString("### 上一轮思维链\n\n")
					} else {
						sb.WriteString("### Previous Chain of Thought\n\n")
					}
					sb.WriteString(cotDisplay)
					sb.WriteString("\n\n")
				}

				// Display decision JSON (if exists)
				if decisionJSON, ok := recordMap["decision_json"].(string); ok && decisionJSON != "" {
					// Limit decision JSON length
					maxJSONLength := 1000
					...
					if len(jsonDisplay) > maxJSONLength {
						if lang == LangChinese {
							...
							jsonDisplay = jsonDisplay[:maxJSONLength] + "\n... (Decision JSON truncated, showing first 1000 characters)"
						}
					}
					if lang == LangChinese {
						sb.WriteString("### 上一轮决策摘要\n\n")
					} else {
						sb.WriteString("### Previous Decision Summary\n\n")
					}
					sb.WriteString("```json\n")
		...
		if lang == LangChinese {
			...
			sb.WriteString("**Note: The above is the previous decision context. Please refer to it but do not rely on it completely. Make new decisions based on current market conditions.**\n\n")
		}
		sb.WriteString("---\n\n")
	}
...
func (e *StrategyEngine) formatTimeframeSeriesData(sb *strings.Builder, data *market.TimeframeSeriesData, indicators store.IndicatorConfig) {
	// 仅输出当前周期汇总：Current (this TF) + Last 10 bars + Last 6 bars，不输出各指标明细（K 线表、EMA/MACD/RSI/BOLL/KDJ 等数组）
