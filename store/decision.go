...
// sanitizeUTF8 cleans invalid UTF-8 byte sequences from a string
// This ensures all strings can be safely stored in PostgreSQL UTF-8 database
func sanitizeUTF8(s string) string {
	...
	// This preserves the string structure while making it valid UTF-8
	// Using replacement character instead of empty string to preserve information
...
func (s *DecisionStore) LogDecision(record *DecisionRecord) error {
	...
	// This prevents PostgreSQL "invalid byte sequence" errors
	dbRecord := &DecisionRecordDB{
		TraderID:            sanitizeUTF8(record.TraderID),
		CycleNumber:         record.CycleNumber,
		Timestamp:           record.Timestamp,
		SystemPrompt:        sanitizeUTF8(record.SystemPrompt),
		InputPrompt:         sanitizeUTF8(record.InputPrompt),
		CoTTrace:            sanitizeUTF8(record.CoTTrace),
		DecisionJSON:        sanitizeUTF8(record.DecisionJSON),
		RawResponse:         sanitizeUTF8(record.RawResponse),
		CandidateCoins:      sanitizeUTF8(string(candidateCoinsJSON)),
		ExecutionLog:        sanitizeUTF8(string(executionLogJSON)),
		Decisions:           sanitizeUTF8(string(decisionsJSON)),
